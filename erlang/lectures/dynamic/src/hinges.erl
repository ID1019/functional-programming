...
%%% and latches (haspar) given a set of requirements. We have a
...
%%% are sold at different prices. Find out how many hinges and latches
%%% that one should produce in order to maximize profit.
...
%%% bring the compexity down to O(n^2) (or more precise O(m*t) ).
...
search(M, T, {Hm, Ht, Hp}=H, {Lm, Lt, Lp}=L, Mem) when (M >= Hm) and (T >= Ht) and (M >= Lm) and (T >= Lt) ->
    ...
    if 
	(Pi+Hp) > (Pj+Lp) ->
	    {(Hi+1), Li, (Pi+Hp), store({M,T}, {(Hi+1), Li, (Pi+Hp)}, Upd2)};
	true ->
	    {Hj, (Lj+1), (Pj+Lp), store({M,T}, {Hj, (Lj+1), (Pj+Lp)}, Upd2)}
    end;
search(M, T, {Hm, Ht, Hp}=H, L, Mem) when ((M >= Hm) and (T >= Ht))  ->
    {Hn, Ln, P, Upd} = check((M-Hm), (T-Ht), H, L, Mem),
    {Hn+1, Ln, (P+Hp), store({M,T}, {Hn+1, Ln, (P+Hp)}, Upd)};
search(M, T, H, {Lm, Lt, Lp}=L, Mem) when ((M >= Lm) and (T >= Lt)) ->
    {Hn, Ln, P, Upd} = check((M-Lm), (T-Lt), H, L, Mem),
    {Hn, Ln+1, P+Lp,  store({M,T}, {Hn, Ln+1, P+Lp}, Upd)};
search(M, T, _,  _, Mem)  ->
    {0,0,0, store({M,T}, {0,0,0}, Mem)}.
...
bench2(N) ->
    ...
    io:format(Fd, "%~9s ~10s ~10s ~10s ~10s~n", ["M", "T", "N", "list", "tree"]),
    lists:foreach(fun({M,T}) ->
			  erlang:garbage_collect(),
			  {T1, _} = timer:tc(fun() -> memory(M, T, {260, 40, 30}, {180, 60, 24}) end),
			  erlang:garbage_collect(),
			  {T2, _} = timer:tc(fun() -> tree(M, T, {260, 40, 30}, {180, 60, 24}) end),
			  io:format(Fd, "~10w ~10w ~10w ~10w ~10w~n", [M div 1000, T div 1000, ((T+M) div 1000), T1 div 1000, T2 div 1000])
...
bench3(N) ->
    ...
    io:format(Fd, "%~9s ~10s ~10s ~10s ~10s~n", ["M", "T", "N", "tree", "ordered"]),
    lists:foreach(fun({M,T}) ->
			  erlang:garbage_collect(),
