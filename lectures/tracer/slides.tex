...
\begin{frame}[fragile]{tuples and records}
 ...
 \begin{itemize}
  ...
  \item a ray:  a tuple with named properties  (actually {\tt \{:ray, \{0,0,0\}, \{0,0,1\}}, but don't count on it.
 \end{itemize}

 \pause\vspace{20pt}

 A number of macros defined to help us handle properties of rays.
 
\pause \vspace{10pt}
\verb+foo = Ray.ray(pos: {3, 2, 4}, dir: {0.447, 0.894, 0})+

\pause \vspace{10pt}
\verb+Ray.ray(pos: pos) = foo+

\pause \vspace{10pt}
\verb+p = Ray.ray(foo, :pos)+
...
\begin{frame}[fragile]{maps and structs}

  Structs are named maps.

  Gives us a nice syntax to handle the objects.

  \pause\vspace{20pt}

\pause \vspace{10pt}
\verb+foo = %Sphere{pos: {3, 2, 4}, dir: {0.447, 0.894, 0}}+

\pause \vspace{10pt}
\verb+%Sphere{pos: pos} = foo+

\pause \vspace{10pt}
\verb+p = foo.pos+
  
\pause \vspace{10pt}
Note, access is lg(n) of number of properties, not as efficent as tuples nor records.
...
defimpl Object do
 ...
 def intersect(sphere, ray) do
   Ray.ray(ray, pos: o, dir: l)
   k = Vector.sub(sphere.pos, o)
   a = Vector.dot(l, k)
   a2 = :math.pow(a, 2)
...
defmodule Camera do
  ...
  Record.defrecord(:camera, pos: nil, corner: nil, 
...
\begin{verbatim}
  def normal(size) do
    ...
    right = {1, 0, 0}
    down = {0, -1, 0}
    Camera.camera(pos: pos, corner: corner, .... )
  end
\end{verbatim}
