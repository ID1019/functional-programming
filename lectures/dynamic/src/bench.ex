defmodule Bench do
  ...
  def memory()  do
    n = 7  ## this enough, searching will take 30 s
  ...
  def map()  do
    ...
    Enum.each( all,
      fn ({m,t}) ->
	:erlang.garbage_collect()
	{t1, _} = :timer.tc(fn() -> Hinges.memory(m, t, {260, 40, 30}, {180, 60, 24}) end)
	:erlang.garbage_collect()
      {t2, _} = :timer.tc(fn() -> Hinges.map(m, t, {260, 40, 30}, {180, 60, 24}) end)
      :io.format(fd, "~10w ~10w ~10w ~10.2f ~10.2f~n", [m, t, (t+m), t1/1000, t2/1000])
      end)
    :file.close(fd)
  end
