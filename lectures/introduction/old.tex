...
\pause
 \item a set of primite functions: $+$, $-$, $*$, $\mathrm{mod}$,  $\mathrm{div}$ 
\pause
 \item syntax: symbols, precedence, paratheses i.e. a way to write expressions
\end{itemize}
...
\begin{itemize}
\pause \item then all arguments of a function must be evaluated
...
\vspace{20pt}{\em Sofar, functions do not have names.}
...
\pause\vspace{10pt}
by {\em substituting} the parameter (or formal argumnet) of the function with the argument.
...
{\em We can wtite $\lambda x (\rightarrow ( \lambda x -> (x*x)) )$, which does complicate things.}
...
\vspace{20pt}{\em We have to be careful but renaming varaiables solves the problem.}
  ...
  \begin{itemize}
    ...
    \pause \item Easy to define:
      \begin{itemize} 
         ...
         \pause \item (you don't even need datastructures nor named functions)
      \end{itemize}
...
\begin{frame}{Erlang}
 ...
 \begin{itemize}
    ...
    \pause \item an ``eager evaluatuion'' i.e. arguments are evaluated before the function is applied 
 \end{itemize}
