defmodule Church do
  ...
  ## fn(x) -> x + 1 end and let the Church number apply this to zero. 
  ...
  ## Multiplying n and m; return a function that takes two arguments
  ## where the first argument will be included in a function that
  ## applies it m times to an argumnet, and this functhen then is
  ## applied n times on teh second argumnet x.
  ...
  def pred(n) do
    fn(f, x) ->
      ...
      ).(fn(u) -> u end)                          # the resulting function
                                                  # is applied to the identity function
    end
  end


### How does this work? Assume we call pred(four) where four is the
...
### function that ignores its argument. Let's apply this functions
### ones, what do we get?
###
...
###    fn(h) -> h.(0) end
### 
### Ok, so now we have three more goes to apply the strange function.
###
...
### function fn(u) -> u end. The result is:
###
###    f.(f.(f.(0)))
###
...
### If you think this was complicated you're right!
  ...
  ### Warning - this will not work since if_then_else will evaluate
  ### all arguments. 
  ...
  ### This works but now we use the Elixir case statement and thereby
  ### avoid the eager evaluation. Using case is of course cheating. 
  ...
  def church_and(a,b) do a.(b,a) end
  def church_or(a,b) do a.(a,b) end 
  def church_not(a) do
    fn(x,y) -> a.(y, x) end
  end
