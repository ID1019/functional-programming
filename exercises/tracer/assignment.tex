...
a default posistion {\tt \{0,0,0\}} and direction {\tt \{1,1,1\}}. If
none is given when we create a ray the deafult values will be used.
...
that makes it easy to add new obejcts in the future.
...
one functtion {\tt intersetc/2} that will determine if a ray
...
represented by a vector and the radious by a number. We create a
...
declare this as the {\tt Obect} protocol implementation.
...
That completes all the modules that we need to represnt the world,
...
different kinds. We onky have spheres now but can add any objects as
...
of exit an object, the {\em current} refraxtion index and the index of
the material outside of an object. If we only consier air and solid
