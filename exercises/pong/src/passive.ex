defmodule WebSocket do 
  ...
  def connect(listen, session) do
	...
	case handshake(socket, <<>>) do
	  :ok ->
	    handler = spawn_link(fn() -> handler(socket, session) end)
  ...
  defp decoder(socket, handler, rest) do
    case decode(rest) do
      {:ping, msg, rest} ->
  ...
  defp handshake(socket, sofar) do
	...
	case parse_request(request) do
	  ...
	  :more ->
	    # more bytes required
	    :io.format("ws: decoder, more bytes required\n")
  ...
  defp parse_request(request) do
    case :binary.part(request, byte_size(request), -4) do
      "\r\n\r\n" ->
	request = :binary.part(request, byte_size(request) - 4)
