...
\begin{tikzpicture}[>=stealth', node distance=3cm, semithick, auto]
    ...
    \node[initial, state]   (init)                             {init od};
...
\begin{tikzpicture}[scale=1,>=stealth', node distance=3cm, semithick, auto]
    ...
    \path[->]   (init)   edge       [bend left]                  node              {\{:connect, lnk\}}   (receive)
               ...
               (receive)   edge     [loop above, every loop/.append style={looseness=6,in=20, out=80}]   node[anchor=west] {\{:dgr, i\}} (receive)
                         edge       [loop below, every loop/.append style={looseness=6,in=340, out=280}] node[anchor=west] {\{:dgr, j\}\\ if $j<i$}  (receive)
                         edge       []                                                                   node              {:quit}                   (final);
\end{tikzpicture}}
